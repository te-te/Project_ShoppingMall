...
class ConnectModel{
    protected $_dbConnections = array();    // PDO 인스턴스를 배열로 저장해두는 프로퍼티
    protected $_connectName;                // 접속명을 저장하는 프로퍼티
    protected $_modelList = array();        // 실행 클래스(ExecuteModel의 서브 클래스)의
                                            // 인스탄스를 저장하는 프로퍼티
    const MODEL = 'Model';                  // 실행 클래스(ExecuteModel의 서브 클래스)명을
                                            // 지정하기 위한 상수
    ...
    public function connect($name,$connection_strings){
        ...
        // $connection_strings:
        try{
            $cnt = new PDO( // PDO 객체 생성
                $connection_strings['string'],  // 접속 문자열
                $connection_strings['user'],    // DB 사용자명
                $connection_strings['password'] // DB 패스워드
            );
    ...
    public function getModelconnection(){
        if(isset($this->_connectName)){ // _connectName값이 NULL이 아니면 TRUE
            ...
            // 접속명 $name과 관련되어있는 PDO 객체 획득
            $cnt = $this->getConnection($name);
        }else{
            // _connectName값이 NULL이면 제일 앞쪽 PDO 객체 획득
